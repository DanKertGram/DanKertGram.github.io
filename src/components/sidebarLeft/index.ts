...
export class AppSidebarLeft extends SidebarSlider {
  ...
  private async createMoreSubmenu(
    ...
    const btns: ButtonMenuItemOptionsVerifiable[] = [{
      ...
      icon: 'help',
      text: 'DanKertGramFeatures',
      onClick: () => {
        const url = I18n.format('DanKertGramFeaturesUrl', true);
  ...
  public initSearch() {
    ...
    const updatePicked = () => {
      // (this.inputSearch.input as HTMLInputElement).placeholder = pickedElements.length ? 'Search' : 'DanKertGram Search';
...
function getVersionLink() {
  ...
  t.textContent = `DanKertGram Web${App.suffix} ${App.version} (${App.build})`;
