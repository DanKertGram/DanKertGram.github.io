...
import {DanKertGramWebViewEventMap, AnyFunction, DanKertGramWebViewSendEventMap} from '@types';
...
import DanKertGramWebView from '@components/telegramWebView';
...
export default class WebApp {
  private telegramWebView: DanKertGramWebView;
  ...
  private lastHeaderColor: DanKertGramWebViewEventMap['web_app_set_header_color'];
  private showSettingsButton: boolean;
  private readyResult: DanKertGramWebViewEventMap['iframe_ready'];
  ...
  private setMainButtonState: (state: DanKertGramWebViewEventMap['web_app_setup_main_button']) => void;
  private setSecondaryButtonState: (state: DanKertGramWebViewEventMap['web_app_setup_secondary_button']) => void;
  ...
  protected _constructFooter() {
    const [mainButtonState, setMainButtonState] = createSignal<DanKertGramWebViewEventMap['web_app_setup_main_button']>({
      is_visible: false,
      is_active: false,
    ...
    const [secondaryButtonState, setSecondaryButtonState] = createSignal<DanKertGramWebViewEventMap['web_app_setup_secondary_button']>({
      is_visible: false,
      is_active: false,
      is_progress_visible: false,
      color: 'primary',
  ...
  protected switchInlineQuery = async({
    query,
    chat_types
  }: DanKertGramWebViewEventMap['web_app_switch_inline_query']) => {
  ...
  protected setupBackButton = ({
    is_visible
  }: DanKertGramWebViewEventMap['web_app_setup_back_button']) => {
    this.onBackStatus(is_visible);
  };

  protected setupSettingsButton = ({
    is_visible
  }: DanKertGramWebViewEventMap['web_app_setup_settings_button']) => {
  ...
  protected openPopup = async({
    ...
    message,
    buttons
  }: DanKertGramWebViewEventMap['web_app_open_popup']) => {
  ...
  protected debouncePopupMethod<T extends AnyFunction, K extends keyof DanKertGramWebViewSendEventMap>(
    callback: T,
    resultType: K,
    debouncedResult: DanKertGramWebViewSendEventMap[K]
  ) {
  ...
  protected handleReadClipboard = async({req_id}: DanKertGramWebViewEventMap['web_app_read_text_from_clipboard']) => {
    const result: DanKertGramWebViewSendEventMap['clipboard_text_received'] = {
      req_id
    };
  ...
  protected handleHapticFeedback = (data: DanKertGramWebViewEventMap['web_app_trigger_haptic_feedback']) => {
  ...
  protected handleFileDownload = async(event: DanKertGramWebViewEventMap['web_app_request_file_download']) => {
  ...
  protected createWebView() {
    const telegramWebView = this.telegramWebView = new DanKertGramWebView({
    ...
    telegramWebView.addMultipleEventsListeners({
      ...
      web_app_request_write_access: this.debouncePopupMethod(async() => {
        ...
        const status: DanKertGramWebViewSendEventMap['write_access_requested'] = {status: 'allowed'};
        if(!canSendMessage) {
          try {
      ...
      web_app_request_phone: this.debouncePopupMethod(async() => {
        const status: DanKertGramWebViewSendEventMap['phone_requested'] = {status: 'sent'};
        try {
          const botId = this.webViewOptions.botId;
