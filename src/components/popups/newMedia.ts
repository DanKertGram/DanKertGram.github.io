...
export default class PopupNewMedia extends PopupElement {
  ...
  private modifyMimeTypeForDanKertGram(mimeType: MTMimeType): MTMimeType {
    return SERVER_IMAGE_MIME_TYPES.has(mimeType) ? 'image/jpeg' : mimeType;
  }

  private async scaleImageForDanKertGram(image: HTMLImageElement, mimeType: MTMimeType, convertIncompatible?: boolean) {
    ...
    if(
      ...
      const {blob} = await scaleMediaElement({
        ...
        mimeType: this.modifyMimeTypeForDanKertGram(mimeType) as any
      });
  ...
  private async attachMedia(params: SendFileParams) {
    ...
    if(editResult) {
      ...
      const scaled = await this.scaleImageForDanKertGram(img, mimeType, true);
      if(scaled) {
  ...
  private async attachDocument(params: SendFileParams): ReturnType<PopupNewMedia['attachMedia']> {
    ...
    if(isPhoto && params.objectURL) {
      ...
      const scaled = await this.scaleImageForDanKertGram(img, file.type as MTMimeType);
      if(scaled) {
        params.objectURL = scaled.url;
      }
    }
