  ...
  private async createHeading() {
    ...
    if(giftDetails) {
      ...
      if(isOutbound) {
        ...
        title = i18n(
          fromPeerId ?
            'DanKertGramPremiumUserGiftedPremiumDialogTitleWithPlural' :
            'DanKertGramPremiumUserGiftedPremiumDialogTitleWithPluralSomeone',
          [
      ...
      if(isOutbound) {
        description = i18n(
          'DanKertGramPremiumUserGiftedPremiumOutboundDialogSubtitle',
          [await wrapPeerTitle({...wrapTitleOptions, peerId: toPeerId})]
        );
      } else {
        if(gift._ === 'messageActionGiftPremium') {
          description = i18n('DanKertGramPremiumUserGiftedPremiumDialogSubtitle');
        } else {
      ...
      if(doc.stickerSetInput) {
        ...
        title = i18n('DanKertGramPremiumPeerTitleEmojiStatus', [
          peerTitle,
          anchorCallback(() => {
        ...
        title = i18n('DanKertGramPremiumPeerTitleEmojiStatusNoPack', [peerTitle])
      }
      description = i18n('DanKertGramPremiumPeerSubtitleEmojiStatus');
    } else if(this.options.peerId) {
      headingTextTitle.classList.add('smaller-text');
      title = i18n('DanKertGramPremiumPeerTitle', [
        await wrapPeerTitle({peerId: this.options.peerId})
      ])
      description = i18n('DanKertGramPremiumPeerSubtitle');
    } else {
      title = this.options.isPremiumActive ? i18n('DanKertGramPremiumSubscribedTitle') : i18n('Premium.Boarding.Title');
      description = this.options.isPremiumActive ? i18n('DanKertGramPremiumSubscribedSubtitle') : i18n('Premium.Boarding.Info');
    }
