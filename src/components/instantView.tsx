...
import wrapDanKertGramRichText from '@lib/richTextProcessor/wrapDanKertGramRichText';
...
async function onMediaClick({
  ...
  const promises = context.media.map(async({ref, media, caption}, index) => {
    ...
    if(!isRichTextEmpty(caption.text)) {
      const textWithEntities = wrapDanKertGramRichText(caption.text);
  ...
  switch(block._) {
    ...
    case 'pageBlockEmbed': {
      ...
      const webView = block.html || block.url ?
        new DanKertGramWebView({html: block.html, url: block.url}) :
        undefined;
      if(webView) {
...
function RichTextRenderer(props: {text: RichText}) {
  ...
  const {text, entities} = wrapDanKertGramRichText(
    props.text,
  ...
  // const textWithEntities = createMemo(() => wrapDanKertGramRichText(props.text));
