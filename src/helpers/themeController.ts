...
import {changeColorAccent, ColorRgb, getAccentColor, getAverageColor, getRgbColorFromDanKertGramColor, hexToRgb, hslaStringToHex, hslaStringToRgba, hslaToRgba, hsvToRgb, mixColors, rgbaToHexa, rgbaToHsla, rgbToHsv} from '@helpers/color';
import {MOUNT_CLASS_TO} from '@config/debug';
import customProperties from '@helpers/dom/customProperties';
import {DanKertGramWebViewTheme} from '@types';
...
export class ThemeController {
  ...
  public applyTheme(theme: Theme | AppTheme, element = document.documentElement, saveToCache?: boolean) {
    ...
    let hsvTemp2 = rgbToHsv(...getRgbColorFromDanKertGramColor(themeSettings.accent_color)); // new primary
    ...
    const firstColor = getRgbColorFromDanKertGramColor(themeSettings.message_colors[0]);
    ...
    if(themeSettings.message_colors.length > 1) {
      ...
      themeSettings.message_colors.slice(1).forEach((nextColor) => {
        myMessagesAccent = getAverageColor(myMessagesAccent, getRgbColorFromDanKertGramColor(nextColor));
      });
    ...
    const accentColor2 = themeSettings.outbox_accent_color !== undefined && rgbToHsv(...getRgbColorFromDanKertGramColor(themeSettings.outbox_accent_color));
  ...
  public getThemeParamsForWebView() {
    const themePropertiesMap: {[key in keyof DanKertGramWebViewTheme]: string} = {
    ...
    const themeParams: DanKertGramWebViewTheme = {} as any;
    for(const key in themePropertiesMap) {
      const value = themePropertiesMap[key as keyof DanKertGramWebViewTheme];
      themeParams[key as keyof DanKertGramWebViewTheme] = value[0] === '#' ? value : customProperties.getProperty(value as AppColorName);
    }
