...
export function getHexColorFromDanKertGramColor(color: number) {
  const hex = (color < 0 ? 0xFFFFFF + color : color).toString(16);
  return '#' + (hex.length >= 6 ? hex : '0'.repeat(6 - hex.length) + hex);
}

export function getRgbColorFromDanKertGramColor(color: number) {
  return hexToRgb(getHexColorFromDanKertGramColor(color));
}
...
export function getColorsFromWallPaper(wallPaper: WallPaper) {
  ...
  ].filter(Boolean).map(getHexColorFromDanKertGramColor).join(',') : '';
}
