...
import {ConnectionType, constructDanKertGramWebSocketUrl, DcConfigurator, TransportType} from '@lib/mtproto/dcConfigurator';
    ...
    this.rootScope.addEventListener('premium_toggle', (isPremium) => {
      this.iterateNetworkers(({networker, connectionType, dcId, transportType}) => {
        ...
        if((transport as TcpObfuscated).connection) {
          const url = constructDanKertGramWebSocketUrl(dcId, connectionType, isPremium);
