...
const defaults: PushStorage = {
  ...
  push_lang: {
    ...
    push_message_error: 'DanKertGram is syncing in the background...',
...
async function handlePushNotificationObject(obj: PushNotificationObject) {
  ...
  try {
    ...
    const tag = 'fix';
    const notificationPromise = ctx.registration.showNotification('DanKertGram Web', {
...
function fireNotification(
  ...
  let title = obj.title || 'DanKertGram';
