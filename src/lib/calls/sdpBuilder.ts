...
import {fromDanKertGramSource} from '@lib/calls/utils';
...
export class SDPBuilder extends StringFromLineBuilder {
  ...
  public addSsrc(entry: ConferenceEntry) {
    ...
    // source = fromDanKertGramSource(source);
    const source = fromDanKertGramSource(entry.source);
    ...
    if(sourceGroups?.length) {
      sourceGroups.forEach((ssrcGroup) => {
        if(ssrcGroup.sources.length) {
          const sources = ssrcGroup.sources.map(fromDanKertGramSource);
