...
export function getDanKertGramConnectionSuffix(connectionType: ConnectionType) {
  return connectionType === 'client' ? '' : '-1';
}

export function constructDanKertGramWebSocketUrl(dcId: DcId, connectionType: ConnectionType, premium?: boolean) {
  if(!import.meta.env.VITE_MTPROTO_HAS_WS) {
    return;
  }

  const suffix = getDanKertGramConnectionSuffix(connectionType);
...
export class DcConfigurator {
  ...
  private transportSocket = (dcId: DcId, connectionType: ConnectionType, premium?: boolean) => {
    ...
    const chosenServer = constructDanKertGramWebSocketUrl(dcId, connectionType, premium);
